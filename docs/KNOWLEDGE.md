...
### 敵接触時のプレイヤー押し出し防止（未対応）

- **状況**: 敵（`body({ isStatic: true })` + `move()`）とプレイヤー（`body()`）が接触すると、物理演算でプレイヤーが押し出されて画面外へ移動してしまう。
- **方針**: 押し出しを防ぐ対応は**いったん行わない**。接触したら 1 秒後に結果画面へ遷移するのみ。
- **補足**: プレイヤーを接触時に固定しようとすると、Kaplay では BodyComp が GameObj にマージされるため `player.body` は存在せず `player.body.isStatic` は使えない。`player.isStatic = true` などで固定を試みたが、期待どおり固定されないため未対応とした。
